...
class ClipModel(nn.Module):
    ...
    def encode_images(self, pil_images: list, batch_size=64) -> np.ndarray:
        ...
        for i in range(0, len(pil_images), batch_size):
            ...
            batch = self.processor(image = img_batch, return_tensors = "pt")
            img_inputs = {
                key: value for key,value in batch.items() if key in {"pixel_values","pixel_attention_mask","spatial_shapes"}
            }
            img_inputs["pixel_values"] = img_inputs[
                "pixel_values"
            ].to(dtype=self.model_dtype).to(self.device) # Only pixel_values need to transform in fp16/fp32
        ...
        return np.concatenate(feats,axis =0) if feats else np.zeros((0, self.dim),self.model_dtype)
    ...
    def encode_texts(self, texts: list[str], batch_size=256) -> np.ndarray:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            with torch.inference_mode():
                ...
                features = F.normalize(features,dim = -1)
                feats.append(features.float().cpu().numpy().astype(self.model_dtype))
    
        return np.concatenate(feats, 0) if feats else np.zeros((0, self.dim), (self.model_dtype))
